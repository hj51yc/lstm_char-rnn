...
class LSTM(object):
    ...
    def grad_clip_all(self, dw_dict, rescale=10.0):
        norm = 0.0
        for dw_name in dw_dict:
            norm += np.sum(np.abs(dw_dict[dw_name]))
        if norm > rescale:
            for dw_name in dw_dict:
                dw_dict[dw_name] *= (rescale / norm)
        return
    ...
    def backward(self, prob, y_label, d_next, cache):
        ...
        grad = dict(Wf=dWf, Wi=dWi, Wo=dWo, Wc=dWc, bf=dbf, bi=dbi, bc=dbc, bo=dbo, by=dby, dh=new_dh_next, dc=new_dc_next)
        self.grad_clip_all(grad)
        new_d_next = (grad['dh'], grad['dc']) 
        del grad['dh']
        del grad['dc']
